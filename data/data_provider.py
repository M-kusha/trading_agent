...
class DataProviderStatus:
    ...
    instruments_available: List[str] = None
    timeframes_available: List[str] = None

    def __post_init__(self):
        if self.instruments_available is None:
            self.instruments_available = []
        if self.timeframes_available is None:
...
class MarketData:
    ...
    def from_series(cls, series: pd.Series, timestamp: Optional[datetime] = None) -> MarketData:
        ...
        if timestamp is None:
            timestamp = pd.to_datetime(series.name) if series.name else datetime.now()
...
class LiveDataProvider(DataProviderInterface):
    ...
    def __init__(self, login: int, password: str, server: str, instruments: List[str], timeframes: List[str]):
        self.login = login
