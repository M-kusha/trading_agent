...
#     intensity: float  # 0.0..1.0 magnitude (sign handled when mapping to buy/sell)
#     size: float       # notional size in EUR (requested)
#     confidence: float
...
#       • No broker/MT5 calls here. No balance/trade publishing here.
#       • Returns keep contract compatibility; execution snapshots are read-only/optional.
#     """
...
#         # Typed config alias (self.C) + dict mirror (self.config)
...
#         self._sync_from_bus_env()  # align with env's environment_config (balance, instruments)
...
#                 details="Decider-only (Env/Executor execute)",
#             )
#         )
#         self._flush_logs()
...
#         # Mirror of ENV/Executor positions snapshot so PM can reason about scale/close
...
#         """
#         Align PM config with the env's published environment_config (initial balance, instruments).
#         Called during init/reinit. Does not publish anything.
#         """
#         try:
...
#             # adopt instruments from env unless user forced them via constructor
...
#             self.default_max_pct = self.C.max_position_pct
#         except Exception:
#             # non-fatal; keep local config
#             pass

#     def _read_balance_and_drawdown(self) -> Tuple[float, float]:
#         """
#         Read balance & drawdown with priority:
#           environment_config → (optional) market_state → (optional) portfolio_metrics
#         NOTE: market_state/portfolio_metrics are Executor outputs; we don't require them.
#         """
...
#     ) -> Dict[str, Any]:
#         """
#         Build a canonical order payload for the Env/Executor to execute.
#         The Executor should convert size_eur into units/lots per symbol rules.
#         """
...
#         """
#         Append orders to the shared 'order_queue' on the bus.
#         Env/Executor should pop/consume/clear as they execute.
#         """
#         if not orders:
#             return
#         try:
...
#     ) -> List[Dict[str, Any]]:
#         """
#         Convert position decisions to concrete orders for the Env/Executor.
#         Size is in EUR notional (Executor maps to units).
#         """
...
#                         instrument=inst, decision=dr.decision.value, size_eur=f"{dr.size:.2f}",
#                         reduce_only=str(reduce_only), confidence=f"{dr.confidence:.3f}"
#                     )
#                 )
...
#             # No market data? return minimal contract payload (hold)
...
#                 # close to deadline: publish decisions, skip heavy work
#                 await self._update_smartbus_with_decisions(decisions)
#                 metrics = self._read_env_metrics()
#                 current_queue = self._safe_get_order_queue()
#                 signals_map = self._signals_map_from_decisions(decisions)
#                 return self._contract_payload(
#                     decisions=decisions,
#                     orders_created=[],
#                     processing_ms=(time.time() - t0) * 1000.0,
#                     thesis="Aborted post-processing due to time budget",
#                     instrument_signals=signals_map,
#                     order_queue=current_queue,
#                     **metrics,
#                 )

#             # Publish decision artifacts (not execution)
#             await self._update_smartbus_with_decisions(decisions)
#             await asyncio.sleep(0)
...
#             # Read the live queue after enqueue to return it explicitly
#             current_queue = self._safe_get_order_queue()

#             # Thesis + metrics for output only
...
#                 "ts": datetime.datetime.utcnow().isoformat() + "Z",
#                 "decisions": {k: {"decision": v.decision.value, "confidence": v.confidence} for k, v in decisions.items()},
...
#     # ─────────────────────────────────────────────────────────
#     # Contract output builder (keeps keys; execution owned by Env/Executor)
#     # ─────────────────────────────────────────────────────────
...
#         instrument_signals: Optional[Dict[str, Any]] = None,  # required by contract
#         order_queue: Optional[List[Dict[str, Any]]] = None,   # required by contract
#     ) -> Dict[str, Any]:
#         position_decisions = {
#             inst: {
...
#             "pending_orders": copy.deepcopy(orders_created),   # proposed this step
...
#         """Read live metrics (if any) strictly from Env/Executor-published bus keys (optional)."""
#         balance, drawdown = self._read_balance_and_drawdown()
#         equity = balance  # equity may be absent; align with balance if so
#         current_pnl = 0.0
...
#     # ─────────────────────────────────────────────────────────
#     # SmartBus I/O (required feeds + optional)
#     # ─────────────────────────────────────────────────────────
...
#             # attach liquidity hints if present
...
#         self.smart_bus.set(
#             "position_decisions", position_decisions_map, module="PositionManager",
...
#                 self.logger.info(
#                     format_operator_message(
#                         "📉", "LOSS_STREAK_REDUCTION", reduction_factor=f"{streak_reduction:.2f}",
#                         consecutive_losses=self.consecutive_losses,
#                     )
#                 )
...
#         """Compute exposure ratio = Σ|notional_eur| / balance from optional Executor bus snapshots."""
...
#             # assume scaling in current direction if we had context; use sign from market_direction
#             return float(np.sign(dr.context.market_direction or 1)) * mag
#         if d == PositionDecision.SCALE_DOWN:
#             return -float(np.sign(dr.context.market_direction or 1)) * mag
#         # close/emergency_close/hold -> neutral (0), but keep sign hint from context if needed
#         return 0.0
