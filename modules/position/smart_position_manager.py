...
class SmartPositionManager:
    ...
    def sync_positions(self, mt5_positions: List[Dict[str, Any]]) -> Dict[str, LivePosition]:
        ...
        for symbol, positions in by_symbol.items():
            ...
            if net_pos and abs(net_pos.lots) > 0.001:
                ...
                # Initialize profit peak if new position
                if symbol not in self._profit_peaks:
                    self._profit_peaks[symbol] = net_pos.unrealized_pnl
                else:
                    # Update peak if higher
                    self._profit_peaks[symbol] = max(
                        self._profit_peaks[symbol], 
                        net_pos.unrealized_pnl
                    )
    ...
    def _calculate_net_position(
        ...
        for pos in positions:
            lots = float(pos.get("volume", pos.get("lots", 0)) or 0)
            entry = float(pos.get("price_open", pos.get("entry_price", 0)) or 0)
            pnl = float(pos.get("profit", pos.get("unrealized_pnl", 0)) or 0)
            open_time = float(pos.get("time", pos.get("open_time", 0)) or 0)
            ...
            total_pnl += pnl
            earliest_time = min(earliest_time, open_time) if open_time > 0 else earliest_time
    ...
    def decide(
        ...
        This is the main entry point for position decisions.
        ...
        if position is None:
            return self._decide_new_position(
                symbol, signal_direction, signal_strength, 
                consensus_confidence, reasons
            )
        ...
        if position.unrealized_pnl <= -cfg.hard_stop_loss_eur:
            reasons.append(f"HARD STOP: Loss €{position.unrealized_pnl:.2f} exceeds -€{cfg.hard_stop_loss_eur:.0f}")
        ...
        if position.unrealized_pnl <= -cfg.soft_stop_loss_eur and signal_against:
            reasons.append(
        ...
        if position.age_hours >= cfg.time_decay_hours and \
           position.unrealized_pnl <= -cfg.time_decay_stop_eur:
            reasons.append(
        ...
        peak_profit = self._profit_peaks.get(symbol, 0)
        ...
        if position.is_profitable and signal_against and \
           signal_strength >= cfg.reversal_signal_threshold and \
           position.unrealized_pnl >= cfg.momentum_exit_profit_eur:
            reasons.append(
        ...
        if signal_aligns:
            return self._decide_scale(
                symbol, position, signal_strength, 
                consensus_confidence, reasons
            )
        
        # ─────────────────────────────────────────────────────
        # Case 4: Signal opposes but not strong enough to close
        # ─────────────────────────────────────────────────────
        if signal_against and signal_strength >= cfg.strong_signal_threshold:
            # Strong reversal signal - close and potentially reverse
            reasons.append(
                f"REVERSAL: Strong opposing signal ({signal_strength:.2f}) "
                f"vs {position.direction} position"
            )
            
            # If we're losing, just close. If profitable, reverse.
    ...
    def _decide_new_position(
        ...
        if signal_strength < cfg.min_signal_strength:
            reasons.append(f"Signal too weak ({signal_strength:.2f} < {cfg.min_signal_strength:.2f})")
            return SmartDecision(
                action=PositionAction.HOLD,
                symbol=symbol,
                confidence=0.5,
        ...
        # This prevents opening new positions when we have many hedged positions
        effective_count = max(len(self._positions), self._actual_mt5_position_count)
        if effective_count >= cfg.max_total_positions:
            reasons.append(f"Max positions reached (net={len(self._positions)}, actual={self._actual_mt5_position_count}, max={cfg.max_total_positions})")
            return SmartDecision(
                action=PositionAction.HOLD,
                symbol=symbol,
                confidence=0.5,
        ...
        lots = min(cfg.default_lot_size * signal_strength, cfg.max_lot_size)
    ...
    def _decide_scale(
        ...
        last_scale = self._last_scale_time.get(symbol, 0)
        scale_cooldown_ok = (time.time() - last_scale) >= cfg.scale_up_cooldown_seconds
        
        if position.unrealized_pnl >= cfg.scale_up_min_profit_eur and \
           signal_strength >= cfg.strong_signal_threshold and \
           scale_cooldown_ok and \
           position.lots < cfg.max_lot_size:
        ...
        if position.unrealized_pnl <= -cfg.scale_down_trigger_loss_eur and \
           position.lots > cfg.default_lot_size:
    ...
    def needs_hedge_cleanup(self, mt5_positions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for symbol, positions in by_symbol.items():
            ...
            if buys and sells:
                ...
                buy_lots = sum(float(p.get("volume", 0) or 0) for p in buys)
                sell_lots = sum(float(p.get("volume", 0) or 0) for p in sells)
    ...
    def get_actual_position_count(self) -> int:
        ...
        This is important for detecting hedge situations where we have many
