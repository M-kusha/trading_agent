...
ACTION_TRANSLATION = {
    ...
    'long': 'BUY', 'buy': 'BUY', 'long_risk_assets': 'BUY',
    'trend_following': 'BUY', 'breakout_long': 'BUY', 'momentum_long': 'BUY',
    'risk_on': 'BUY', 'bullish': 'BUY', 'long_bias': 'BUY',
    'seasonal_long_bias': 'BUY', 'breakout': 'BUY',
    'increase_risk': 'BUY', 'aggressive': 'BUY',  # Risk module bullish signals
    'trend_bullish': 'BUY',  # TrendExpert bullish signal
    # Short/bearish actions  
    'short': 'SELL', 'sell': 'SELL', 'short_risk_assets': 'SELL',
    'safe_haven_rotation': 'SELL', 'volatility_hedging': 'SELL',
    'breakout_short': 'SELL', 'momentum_short': 'SELL',
    'risk_off': 'SELL', 'bearish': 'SELL', 'short_bias': 'SELL',
    'seasonal_short_bias': 'SELL', 'mean_reversion': 'SELL',
    'reduce_risk': 'SELL', 'emergency_stop': 'SELL',  # Risk module bearish signals
    'trend_bearish': 'SELL',  # TrendExpert bearish signal
    # Neutral/gate actions (these should have been filtered by committee, but handle them)
    'abstain': 'HOLD', 'hold': 'HOLD', 'neutral': 'HOLD', 'flat': 'HOLD',
    'proceed': 'HOLD', 'wait': 'HOLD', 'no_action': 'HOLD',
    'caution': 'HOLD', 'halt': 'HOLD', 'continue': 'HOLD', 'confirm': 'HOLD',
    'seasonal_neutral': 'HOLD', 'theme_neutral': 'HOLD',
    'momentum_neutral': 'HOLD', 'trend_neutral': 'HOLD',  # Expert neutral signals
    'session_avoid': 'HOLD', 'session_optimal': 'HOLD',  # Seasonality timing signals
    'high_impact_caution': 'HOLD',  # High-impact event caution
}
...
class SlimVotingKernel(VotingModuleBase):
    ...
    def _module_specific_init(self) -> None:
        ...
        if config:
            ...
            for stage_name in skip_list:
                try:
                    ...
                    self._log_warning(f"Unknown stage to skip: {stage_name}")
        ...
        self._log_info("SlimVotingKernel initialized")
    ...
    async def run_pipeline(
        ...
        Args:
            ...
            force_refresh: Force re-collection of votes even if fresh
        ...
        self.log_info(f"Pipeline run #{self._pipeline_run_count} started: {decision_id}")
        ...
        try:
            ...
            # ===============================================================
            # Read stage outputs from SmartInfoBus
            # (Stages are executed by the main orchestrator)
            # ===============================================================
            
            # Committee results
            self._current_stage = PipelineStage.COMMITTEE
            committee_votes = self.smart_bus.get('committee_votes', name) or []
            ...
            raw_proposals = self.smart_bus.get('raw_proposals', name) or {}
            
            if not committee_votes and not raw_proposals:
                self.log_warning("No votes available from committee - defaulting to ABSTAIN")
                return self._make_abstain_result(decision_id, "No votes from committee")
            
            # Consensus results
            self._current_stage = PipelineStage.CONSENSUS
            consensus_score = float(self.smart_bus.get('consensus_score', name) or 0.5)
            consensus_result = self.smart_bus.get('consensus_result', name) or {
                'consensus_score': consensus_score,
                'consensus_exists': consensus_score >= 0.6,
            }
            agreement_score = float(self.smart_bus.get('agreement_score', name) or consensus_score)
            
            # Collusion results
            self._current_stage = PipelineStage.COLLUSION
            collusion_score = float(self.smart_bus.get('collusion_score', name) or 0.0)
            collusion_detected = bool(self.smart_bus.get('collusion_detected', name) or False)
            
            if collusion_detected:
                self.log_warning(f"Collusion detected! Score: {collusion_score:.2%}")
            ...
            uncertainty_result = self.smart_bus.get('uncertainty_result', name) or {
            ...
            final_decision = self.smart_bus.get('final_decision', name) or committee_decision
            gate_decision = self.smart_bus.get('gate_decision', name) or {'passed': True}
            instrument_signals = self.smart_bus.get('instrument_signals', name) or {}
            ...
            pipeline_time_ms = (datetime.now() - pipeline_start).total_seconds() * 1000
            
            # Determine final action (raw semantic action from voting)
            raw_action = final_decision.get('action', committee_decision.get('action', 'abstain'))
            confidence = final_decision.get('confidence', committee_confidence)
    ...
    def _get_active_instruments(self) -> List[str]:
        ...
        # Try bus first
        instruments = self.bus_get(VotingBusKeys.ACTIVE_INSTRUMENTS, default=None)
        if instruments:
            return instruments
    ...
    def _publish_result(self, result: Dict[str, Any]) -> None:
        ...
        # Publish convenience aliases for downstream consumers
        self.bus_set(
    ...
    async def process(self, **inputs) -> Dict[str, Any]:
        ...
        import datetime
        tick_ts = datetime.datetime.now().isoformat()
        ...
        return {
            ...
            "voting_consensus": consensus_result if isinstance(consensus_result, dict) else {"score": consensus_score},
            "consensus_summary": {"consensus_score": consensus_score, "action": final.get("action", "abstain")},
            ...
            "decision_coordination": {"decision_id": decision_id, "tick_ts": tick_ts},
