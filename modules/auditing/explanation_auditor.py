...
class TradeExplanationAuditor(Module):
    """
    Captures, stores, and provides a summary of the rationale for every trade, 
    including pipeline trace, module inputs, and exceptions for deep debugging 
    and LLM analysis.
    """
    ...
    def step(
        self,
        action: Optional[Dict[str, Any]] = None,       # Now supports dict for detailed actions
        reasoning: Optional[str] = None,
        confidence: Optional[float] = None,
        regime: Optional[str] = None,
        voting: Optional[Dict[str, float]] = None,
        pnl: Optional[float] = None,
        modules_trace: Optional[Dict[str, Any]] = None,   # Each module's contribution
        order_status: Optional[str] = None,               # filled/rejected/cancelled/hold
        exception: Optional[Union[Exception, str]] = None,
        **kwargs
    ):
        """Store a rich explanation after every trade or step."""
        exp = {
            "timestamp": datetime.datetime.now().isoformat(),
            "step": self.step_counter,
            "action": action,                  # dict: symbol, side, size, price, etc.
            ...
            "voting": voting,
            "pnl": pnl,
            "modules_trace": modules_trace,    # dict: module_name  input/decision/score
        ...
        if len(self.explanations) > self.history_len:
            self.explanations.pop(0)
        if self.debug:
            print(f"[ExplanationAuditor] {exp}")
    ...
    def get_state(self):
        return {"explanations": self.explanations.copy(), "step_counter": self.step_counter}
