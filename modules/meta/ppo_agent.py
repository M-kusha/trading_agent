...
class PPOAgent(BaseModule, SmartInfoBusTradingMixin, SmartInfoBusRiskMixin, SmartInfoBusStateMixin):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Initialize BaseModule with a plain dict config (avoid assigning to BaseModule.config with a dataclass)
    ...
    def _initialize(self):
        ...
        try:
            ...
            # Be defensive in case logger isn't ready yet
        ...
        try:
            ...
            if 'agent_performance' not in result:
                result['agent_performance'] = {
                    ...
                    'learning_rate': agent_metrics.get('learning_rate', self.training_stats.get('learning_rate', self._cfg.learning_rate))
                }
            # 3) training_metrics
            ...
            if 'policy_gradients' not in result:
                ...
                result['policy_gradients'] = {
                    'gradient_norm': self.training_stats.get('gradient_norm', 0.0),
                    'learning_rate': self.training_stats.get('learning_rate', self._cfg.learning_rate),
    ...
    async def _process_training(self, ppo_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(self.buffer['observations']) >= self._cfg.batch_size:
    ...
    async def _perform_policy_update(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for _ in range(self._cfg.ppo_epochs):
                ...
                surr2 = torch.clamp(ratio, 1 - self._cfg.clip_eps, 1 + self._cfg.clip_eps) * advantages
                ...
                # Total loss
                loss = policy_loss + self._cfg.value_coeff * value_loss + self._cfg.entropy_coeff * entropy_loss
                ...
                # Gradient clipping
                grad_norm_t = torch.nn.utils.clip_grad_norm_(self.network.parameters(), self._cfg.max_grad_norm)
    ...
    def _compute_gae_returns(self):
        ...
        for t in reversed(range(len(rewards))):
            ...
            last_gae = delta + self._cfg.gamma * self._cfg.gae_lambda * next_non_terminal * last_gae
    ...
    def _update_action_statistics(self):
        ...
        if len(self.action_history) > 10:
            ...
            self.action_statistics['exploration_level'] = float(np.clip(action_entropy / self._cfg.act_size, 0, 1))
    ...
    async def _update_agent_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(self.episode_rewards) > 0:
                ...
                performance_score = max(0.0, min(1.0, (avg_reward + 100.0) / 200.0))  # Normalize to 0-1
            else:
                avg_reward = 0.0
                performance_score = 0.0
        ...
        try:
            ...
            if result.get('action_selected', False):
                action_data = {
                    ...
                    'log_prob': result.get('log_prob', 0.0),
                    'value_estimate': result.get('value_estimate', 0.0),
                    'action_std': result.get('action_std', self._last_action_std or [1.0] * int(self._cfg.act_size)),
                    'exploration_level': self.action_statistics['exploration_level']
                }
            ...
            performance_data = {
                ...
                'learning_rate': agent_metrics.get('learning_rate', self._cfg.learning_rate)
            }
    ...
    def confidence(self, obs: Any = None, **kwargs) -> float:
        ...
        performance_confidence = max(0.0, min(1.0, (self.training_stats['avg_episode_reward'] + 50.0) / 100.0))
    ...
    async def propose_action(self, **inputs) -> Dict[str, Any]:
        ...
        try:
            ...
            if obs is not None:
                ...
                # Fallback action
                fallback_action = getattr(self, 'last_action', np.zeros(self._cfg.act_size, dtype=np.float32))
                return {
                    'action_type': 'fallback_action',
                    'action': fallback_action.tolist() if hasattr(fallback_action, 'tolist') else [0.0] * self._cfg.act_size,
                    'confidence': 0.3,
    ...
    async def calculate_confidence(self, action: Dict[str, Any], **inputs) -> float:
        ...
        try:
            ...
            base_confidence = max(0.0, min(1.0, (self.training_stats['avg_episode_reward'] + 50.0) / 100.0))
            ...
            if len(self.policy_losses) > 10:
                recent_losses = list(self.policy_losses)[-10:]
