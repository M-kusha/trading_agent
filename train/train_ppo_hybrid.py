...
# # ───────────────────────────────────────────────────────────────────
# # Minimal provider bootstrap for SmartInfoBus keys used at training start
# # ───────────────────────────────────────────────────────────────────
# def bootstrap_training_providers() -> None:
#     """Instantiate lightweight providers that publish baseline keys at init.

#     Ensures the following keys are available immediately to TrainingScript:
#       - market_overview (ExplanationGenerator)
#       - market_thesis (ThesisEvolutionEngine)
#       - time_risk_health (TimeAwareRiskScaling)
#       - regime_matrix_health (RegimePerformanceMatrix)

#     Safe to call multiple times; failures are swallowed to keep training robust.
#     """
#     try:
#         # Local imports to avoid mandatory dependency if modules package is unavailable
#         from modules.strategy.explanation_generator import ExplanationGenerator  # type: ignore
#         from modules.strategy.thesis_evolution_engine import ThesisEvolutionEngine  # type: ignore
#         from modules.market.time_aware_risk_scaling import TimeAwareRiskScaling  # type: ignore
#         from modules.market.regime_performance_matrix import RegimePerformanceMatrix  # type: ignore
#         from modules.features.advanced_feature_engine import AdvancedFeatureEngine  # type: ignore
#         from modules.external.market_data_provider import MarketDataProvider  # type: ignore
#         from modules.position.position import PositionManager  # type: ignore
#     except Exception:
#         # Modules not available (fallback bus mode)
#         return

#     # Create instances; their _initialize methods publish baseline bus keys
#     for ctor in (ExplanationGenerator, ThesisEvolutionEngine, TimeAwareRiskScaling, RegimePerformanceMatrix, AdvancedFeatureEngine, MarketDataProvider, PositionManager):
#         try:
#             # PositionManager expects TradingConfig-like; pass minimal dict-safe config
#             if ctor.__name__ == "PositionManager":
#                 ctor(config={})  # type: ignore[call-arg]
#             else:
#                 ctor(config={})  # type: ignore[call-arg]
#         except Exception:
#             # Never break training startup on bootstrap errors
#             pass
...
#     instruments = getattr(config, "instruments", None) or ["EUR/USD"]
...
#     # Ensure baseline SmartInfoBus keys exist before inspector attaches
#     try:
#         bootstrap_training_providers()
#     except Exception:
#         pass
...
"""
Complete Modern SmartInfoBus v4.0 Training Script
Clean: no module bootstrapping, no file-based data loading
— it only starts training.

Adds optional Dependency Inspector monitoring:
- Attaches live taps (if available)
- Emits "Training start" and "Training end" reports
- Never blocks training if unavailable or errors occur
"""
...
# Enhanced callback (self-contained; safe fallbacks inside)
from train.enhanced_training_callback import ModernEnhancedTrainingCallback

# ───────────────────────────────────────────────────────────────────
# DATA PROVIDER (SmartInfoBus only + dummy fallback)
# ───────────────────────────────────────────────────────────────────
class DataProvider(Protocol):
    def load(self, config: TradingConfig) -> Dict[str, Dict[str, pd.DataFrame]]:
        ...
...
class OrchestratorOrDummyProvider:
    ...
    def load(self, config: TradingConfig) -> Dict[str, Dict[str, pd.DataFrame]]:
        ...
        try:
            bus = InfoBusManager.get_instance()
            instruments = getattr(config, "instruments", []) or ["EUR/USD"]
            timeframes = getattr(config, "timeframes", None) or ["H1"]
            ...
            for inst in instruments:
                for tf in timeframes:
                    ...
                    blob = None
                    try:
                        blob = bus.get(key, module="TrainingScript", default=None)
                    except Exception:
                        blob = None
                    if not blob:
                        continue
                    try:
                        ...
                        # Ignore malformed blobs; dummy fallback will cover
                        pass
        except Exception:
            pass
...
def train_modern_ppo(config: TradingConfig, pretrained_model_path: Optional[str] = None):
    ...
    # Data (SmartInfoBus if present, else dummy)
    provider = OrchestratorOrDummyProvider()
    data = provider.load(config)
...
def main():
    ...
    # ─────────────────────────────────────────────────────────
    # Dependency monitor (optional, safe if module missing)
    # ─────────────────────────────────────────────────────────
    bus_inspector = None
    try:
        if DependencyInspector_Cls is not None and ENHANCED_LOGGING:
        ...
        try:
            print(f"[WARN] Dependency Inspector initialization failed: {e}")
        except Exception:
            pass
    ...
    try:
        train_modern_ppo(config, pretrained_model_path=pretrained_path)
        print("TRAINING COMPLETED SUCCESSFULLY!")

        # Dependency monitor finalization
        if bus_inspector:
            try:
                if hasattr(bus_inspector, "emit_report"):
                    bus_inspector.emit_report("Training end")
        ...
        print(f"Training failed: {e}")
        # Try to finalize the monitor even on failure (best-effort)
        try:
            if bus_inspector and hasattr(bus_inspector, "emit_report"):
                bus_inspector.emit_report("Training end (failed)")
        except Exception:
            pass
        sys.exit(1)
