...
class RegimeSkillAssessment:
    ...
    def from_trades(cls, trades: List[TradeWithRegime]) -> "RegimeSkillAssessment":
        ...
        assessment.confidence = min(1.0, len(trades) / 100)
        ...
        # Bucket trades
        for trade in trades:
            assessment.volatility_performance[trade.volatility_regime].add_trade(trade)
            assessment.trend_performance[trade.trend_regime].add_trade(trade)
            assessment.session_performance[trade.session_regime].add_trade(trade)
            assessment.spread_performance[trade.spread_regime].add_trade(trade)
    ...
    def _compute_adaptation_score(self) -> None:
        ...
        for perf_dict in [
            ...
            for perf in perf_dict.values():
                if perf.trade_count >= 5:  # Minimum sample
    ...
    def _compute_volatility_handling(self) -> None:
        ...
        if not high_vol or high_vol.trade_count < 5:
        ...
        if low_vol and low_vol.trade_count >= 5:
    ...
    def _compute_trend_following(self) -> None:
        ...
        if not strong_trend or strong_trend.trade_count < 5:
        ...
        if ranging and ranging.trade_count >= 5:
    ...
    def _compute_session_awareness(self) -> None:
        ...
        if not overlap or overlap.trade_count < 5:
        ...
        if off_hours and off_hours.trade_count >= 5:
    ...
    def _compute_cost_resilience(self) -> None:
        ...
        if not wide or wide.trade_count < 5:
        ...
        if tight and tight.trade_count >= 5:
...
def classify_bar_regime(
    ...
    if atr_history and len(atr_history) >= 10:
        atr_percentile = sum(1 for x in atr_history if x <= atr) / len(atr_history)
    else:
        atr_percentile = 0.5
    
    # Spread percentile
    if spread_history and len(spread_history) >= 10:
        spread_percentile = sum(1 for x in spread_history if x <= spread) / len(spread_history)
    else:
